...
def test_dhcp():
    ...
    assert dev1.entity.addresses == {HWAddress("1:0:0:0:0:1"), IPAddress.new("192.168.0.1")}

    # IP reassigned
    f3 = m.connection(IPFlow.UDP("1:0:0:0:0:5", "0.0.0.0", 68) >> ("ff:ff:ff:ff:ff:ff", "255.255.255.255", 67))
    h2 = sb.system.get_endpoint(HWAddress("1:0:0:0:0:5"))
    ...
    assert dev1.entity.addresses == {HWAddress("1:0:0:0:0:1")}
    assert h2.addresses == {HWAddress("1:0:0:0:0:5"), IPAddress.new("192.168.0.1")}
    ...
    h3 = sb.system.get_endpoint(HWAddress("1:0:0:0:0:6"))
    assert h3.name == "1:0:0:0:0:6"
    m.connection(IPFlow.UDP("1:0:0:0:0:6", "192.168.0.1", 68) << ("1:0:0:0:0:2", "192.168.0.2", 67))

    assert dev1.entity.addresses == {HWAddress("1:0:0:0:0:1")}
    assert h2.addresses == {HWAddress("1:0:0:0:0:5")}
    assert h2.name == "192.168.0.1 1"
    assert h3.addresses == {HWAddress("1:0:0:0:0:6"), IPAddress.new("192.168.0.1")}
