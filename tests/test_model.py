...
def test_connection_match():
    ...
    cs = m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1100) >> ("a:0:0:0:0:2", "192.168.0.2", 1234))
    ...
    flow = IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1100) >> ("a:0:0:0:0:3", "1.0.0.3", 1234)
    ...
    cs = m.connection(IPFlow.UDP("a:0:0:0:0:4", "192.168.0.4", 2004) >> ("a:0:0:0:0:5", "1.0.0.5", 2005))
    assert cs is not None
    assert cs.status == Status.EXTERNAL
    assert cs.source.name == "0a:00:00:00:00:04"
    assert cs.target.name == "1.0.0.5"

    cs = m.connection(IPFlow.UDP("a:0:0:0:0:6", "1.0.0.6", 2006) >> ("a:0:0:0:0:2", "192.168.0.2", 1234))
    assert cs is not None
    assert cs.status == Status.UNEXPECTED
...
def test_match_mix_unknown():
    ...
    cs1 = m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1101) >> ("a:0:0:0:0:3", "192.168.0.3", 1234))
    cs2 = m.connection(IPFlow.UDP("a:0:0:0:0:4", "192.168.0.4", 1102) >> ("a:0:0:0:0:3", "192.168.0.3", 1234))
    cs3 = m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1103) >> ("a:0:0:0:0:3", "192.168.0.3", 1234))
    cs4 = m.connection(IPFlow.UDP("a:0:0:0:0:4", "192.168.0.4", 1104) >> ("a:0:0:0:0:3", "192.168.0.3", 1234))
    cs4_2 = m.connection(IPFlow.UDP("a:0:0:0:0:4", "192.168.0.4", 1104) << ("a:0:0:0:0:3", "192.168.0.3", 1234))
    cs5 = m.connection(IPFlow.UDP("a:0:0:0:0:4", "192.168.0.4", 1105) >> ("a:0:0:0:0:3", "192.168.0.3", 2000))
    cs6 = m.connection(IPFlow.UDP("a:0:0:0:0:4", "192.168.0.4", 1106) >> ("a:0:0:0:0:3", "192.168.0.3", 1234))
    cs7 = m.connection(IPFlow.UDP("a:0:0:0:0:7", "192.168.0.7", 1107) >> ("a:0:0:0:0:3", "192.168.0.3", 1234))
    ...
    assert cs1.target.name == "0a:00:00:00:00:03"
    assert cs2.source.name == "0a:00:00:00:00:04"
    assert cs2.target.name == "UDP:1234"  # because there was reply
    assert cs7.source.name == "0a:00:00:00:00:07"
    assert cs7.target.name == "UDP:1234"

    dev1 = sb.system.get_endpoint(HWAddress.new("a:0:0:0:0:1"))
    assert dev1.get_expected_verdict() == Verdict.INCON  # would be updated by inspector
    dev_s = sb.system.get_endpoint(HWAddress.new("a:0:0:0:0:3")).get_entity("UDP:1234")
    assert dev_s == cs2.target
    assert dev_s.get_expected_verdict() == Verdict.INCON  # would be updated by inspector
    dev4 = sb.system.get_endpoint(HWAddress.new("a:0:0:0:0:4"))
    assert dev4 == cs2.source
    assert dev4.get_expected_verdict() == Verdict.INCON
    dev7 = sb.system.get_endpoint(HWAddress.new("a:0:0:0:0:7"))
...
def test_match_overlap_unknowns():
    ...
    m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1001) >> ("a:0:0:0:0:3", "192.168.0.3", 2001))
    m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1001) >> ("a:0:0:0:0:3", "192.168.0.3", 2002))
    m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1002) >> ("a:0:0:0:0:3", "192.168.0.3", 2001))
    m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1002) >> ("a:0:0:0:0:3", "192.168.0.3", 2002))
    # one reply
    m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1001) << ("a:0:0:0:0:3", "192.168.0.3", 2002))
    ...
    cs11 = m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1001) >> ("a:0:0:0:0:3", "192.168.0.3", 2001))
    cs12 = m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1001) >> ("a:0:0:0:0:3", "192.168.0.3", 2002))
    cs13 = m.connection(IPFlow.UDP("a:0:0:0:0:1", "192.168.0.1", 1001) >> ("a:0:0:0:0:3", "192.168.0.3", 2003))
    cs14 = m.connection(IPFlow.UDP("a:0:0:0:0:2", "192.168.0.2", 1001) >> ("a:0:0:0:0:3", "192.168.0.3", 2002))
...
def test_match_local_and_remote():
    ...
    # unknown local first
    cs01 = m.connection(IPFlow.UDP("a:0:0:0:0:3", "192.168.0.3", 1001) >> ("a:0:0:0:3:1", "192.168.1.1", 2001))
    cs02 = m.connection(IPFlow.UDP("a:0:0:0:0:3", "192.168.0.3", 1001) >> ("a:0:0:0:3:1", "19.168.3.2", 2002))

    # unknown remote first
    cs11 = m.connection(IPFlow.UDP("a:0:0:0:0:3", "192.168.0.3", 1001) >> ("a:0:0:0:2:1", "19.168.2.2", 2002))
    cs12 = m.connection(IPFlow.UDP("a:0:0:0:0:3", "192.168.0.3", 1001) >> ("a:0:0:0:2:1", "192.168.1.1", 2001))

    # known local first
    cs21 = m.connection(IPFlow.UDP("a:0:0:0:0:3", "192.168.0.3", 1001) >> ("a:0:0:0:0:1", "192.168.0.1", 2001))
    cs22 = m.connection(IPFlow.UDP("a:0:0:0:0:3", "192.168.0.3", 1001) >> ("a:0:0:0:0:1", "192.168.0.2", 2002))

    assert cs01.target.name == "0a:00:00:00:03:01"
    assert cs02.target.name == "19.168.3.2"
    assert cs11.target.name == "19.168.2.2"
    assert cs12.target.name == "0a:00:00:00:02:01"
...
def test_unknown_multicast():
    ...
    cs1 = m.connection(IPFlow.UDP(
        "a:0:0:0:0:1", "192.168.0.1", 1100) >> ("ff:ff:ff:ff:ff:ff", "255.255.255.255", 1234))
    ...
    assert cs1.source.name == "0a:00:00:00:00:01"
    ...
    cs4 = m.connection(IPFlow.UDP(
        "a:0:0:0:0:2", "192.168.0.2", 1100) >> ("ff:ff:ff:ff:ff:ff", "255.255.255.255", 1234))

    assert cs1 != cs4

    cs5 = m.connection(IPFlow.UDP(
        "a:0:0:0:0:1", "192.168.0.1", 1100) >> ("ff:ff:ff:ff:ff:ff", "255.255.255.255", 2111))
    assert cs1 == cs5
...
def test_unknown_ip_protocol():
    ...
    cs1 = m.connection(IPFlow.UDP(
        "a:0:0:0:0:1", "192.168.0.1", 1) >> ("a:0:0:0:0:2", "192.168.0.2", 1))
...
def test_external_connection():
    ...
    cs1 = m.connection(IPFlow.UDP(
        "20:0:0:0:0:1", "192.168.10.1", 2000) >> ("a:0:0:0:0:2", "192.168.0.2", 1234))
    cs2 = m.connection(IPFlow.UDP(
        "20:0:0:0:0:1", "192.168.10.1", 2000) << ("a:0:0:0:0:2", "192.168.0.2", 1234))
    assert cs1 == cs2
    ...
    cs3 = m.connection(IPFlow.UDP(
        "20:0:0:0:0:1", "192.168.10.1", 2000) >> ("a:0:0:0:0:2", "192.168.0.2", 2001))
    cs4 = m.connection(IPFlow.UDP(
        "20:0:0:0:0:1", "192.168.10.1", 2000) << ("a:0:0:0:0:2", "192.168.0.2", 2001))
    assert cs3 == cs4
    ...
    cs5 = m.connection(IPFlow.UDP(
        "a:0:0:0:0:2", "192.168.10.2", 8000) >> ("30:0:0:0:0:4", "192.168.0.4", 2001))
...
def test_wildcard_source():
    ...
    cs1 = m.connection(IPFlow.TCP(
        "a:0:0:0:0:1", "192.168.10.1", 2000) >> ("a:0:0:0:0:2", "203.0.113.1", 22))
...
def test_any_host():
    ...
    cs4 = m.connection(IPFlow.UDP(
        "a:0:0:0:0:5", "192.168.10.5", 2002) >> ("a:0:0:0:0:2", "192.168.20.10", 1003))
    assert cs4.status == Status.EXTERNAL
    assert cs4.source.name == "0a:00:00:00:00:05"
    assert cs4.target.name == "UDP:1003"

    cs5, sad, tad, reply = m.connection_w_ends(IPFlow.UDP(
        "a:0:0:0:0:1", "192.168.10.1", 2002) >> ("a:0:0:0:0:2", "192.168.20.10", 1004))
    assert cs5.status == Status.UNEXPECTED
    assert cs5.source == dev1.entity
    assert cs5.target.name == "0a:00:00:00:00:02"  # not the any()
    assert sad == EndpointAddress.hw("a:0:0:0:0:1", Protocol.UDP, 2002)
    assert tad == EndpointAddress.hw("a:0:0:0:0:2", Protocol.UDP, 1004)

    # global IP - same HW (must be unknown gateway)
    cs6, sad, tad, reply = m.connection_w_ends(IPFlow.UDP(
        "a:0:0:0:0:1", "192.168.10.1", 2002) >> ("a:0:0:0:0:2", "22.168.20.10", 1002))
...
def test_match_preferences():
    ...
    c = m.connection(IPFlow.UDP(
      "a:0:0:0:0:1", "192.168.10.1", 4000) >> ("a:0:0:0:0:2", "1.0.0.2", 2002))
    assert c.target.parent == p1.entity

    # match P1 host
    c = m.connection(IPFlow.UDP(
      "a:0:0:0:0:1", "192.168.10.1", 4000) >> ("a:0:0:0:0:2", "1.0.0.2", 2001))
    assert c.target == p1.entity

    # matched P1, not P3 wildcard service
    c = m.connection(IPFlow.UDP(
      "a:0:0:0:0:1", "192.168.10.1", 4000) >> ("a:0:0:0:0:2", "1.0.0.2", 2000))
    assert c.target == p1.entity

    c = m.connection(IPFlow.UDP(
      "a:0:0:0:0:2", "192.168.10.2", 4000) >> ("a:0:0:0:0:2", "1.0.0.1", 2000))
    assert c.target == p2.entity

    c = m.connection(IPFlow.UDP(
      "a:0:0:0:0:1", "192.168.10.1", 4000) >> ("a:0:0:0:0:2", "1.0.0.1", 2001))
    assert c.target == p2.entity

    c = m.connection(IPFlow.UDP(
      "a:0:0:0:0:1", "192.168.10.1", 4000) >> ("a:0:0:0:0:2", "1.0.0.3", 2001))
    assert c.target.name == "1.0.0.3"


def test_reply_misinterpretation():
    sb = SystemBackend()
    dev = sb.device().hw("a:0:0:0:0:1")
    m = SystemMatcher(sb.system)

    c0 = m.connection(IPFlow.UDP(
      "a:0:0:0:0:2", "192.168.0.2", 4000) >> ("a:0:0:0:0:1", "192.168.0.1", 2001))

    # this it NOT a reply, but unfortunate ICMP
    c1 = m.connection(IPFlow.IP(
        "a:0:0:0:0:2", "192.168.0.2", 1) << ("a:0:0:0:0:1", "192.168.0.1", 1))
