...
In this setup, tools were operated by sending commands from the computer to the WiFi hotspot over SSH.
...
TDSAF verifies security statements by analyzing and comparing them to data collected from the system with any of the [supported tools](../Tools.md#list-of-supported-tools). When running a security statement file, tool output is provided to TDSAF with command-line flags `-r` and `--read`. Here's an example:
```shell
...
== Connections ==
    ...
    ...
```
The output is divided into two main sections: _Hosts_ and _Connections_. _Hosts_ lists all the hosts identified by TDSAF from the tool output, while _Connections_ provides a list of connections found in the collected data.
