...
# mypy: disable-error-code="valid-type,misc,assignment,arg-type,return-value"
...
class SQLDatabase(EntityDatabase, ModelListener):
    ...
    def _fill_cache(self) -> None:
        ...
        with Session(self.engine) as ses:
            ...
            for src in ses.execute(sel).yield_per(1000).scalars():
                self.free_source_id = max(self.free_source_id, src.id) # type: ignore[call-overload]
    ...
    def _form_event(self, data: Tuple[str, EvidenceSource, str, str]) -> Optional[Event]:
        ...
        event = event_type.decode_data_json(evi, js, self.get_entity) # type: ignore[attr-defined]
