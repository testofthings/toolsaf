...
class BatchImporter:
    ...
    def __init__(self, interface: EventInterface, label_filter: 'LabelFilter' = None, # type: ignore[assignment]
    ...
    def _do_process_files(self, files: List[pathlib.Path], info: 'FileMetaInfo', tool: ToolDepiction,
                          skip_processing: bool) -> None:
        """Process files"""
        if (reader := tool.create_tool(self.system)) is None:
            return
        reader.load_baseline = info.load_baseline or self.load_baseline
        ...
        unmapped = set(reader.file_name_map.keys()) # type: ignore[attr-defined]
        for fn in files:
            if not fn.is_file():
