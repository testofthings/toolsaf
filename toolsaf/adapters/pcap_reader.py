...
from toolsaf.common.address import DNSName, HWAddress, Protocol, IPAddress, HWAddresses
...
from toolsaf.common.traffic import IPFlow, EvidenceSource, Evidence, EthernetFlow

NULL_HW = HWAddress.new("00:00:00:00:00:00")
...
class PCAPReader(SystemWideTool):
    ...
    def process_file(
        self, data: BufferedReader, file_name: str, interface: EventInterface, source: EvidenceSource
    ) -> bool:
        self.source = source
    ...
    def _udp_frame(self, ip: IPv4, frame: UDP, src_hw: Any, dst_hw: Any) -> None:
        ...
        s, d = self.ip_flow_ends(ip, UDP.Source_port[frame], UDP.Destination_port[frame], src_hw, dst_hw)
    ...
    def _tcp_frame(self, ip: IPv4, frame: TCP, src_hw: Any, dst_hw: Any) -> None:
        ...
        if TCP.Flags[frame] & TCPFlag.SYN:
            ...
            s, d = self.ip_flow_ends(ip, TCP.Source_port[frame], TCP.Destination_port[frame], src_hw, dst_hw)
